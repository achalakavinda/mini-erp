        ...
        <!-- general form elements -->
            <div class="box box-primary">
                            ...
                            <div class="form-group">
                                ...
                                <select name="supplier_id" class="form-control">
                                    ...
                                    @foreach(\App\Models\Ims\Supplier::all() as $supplier)
                                        <option @if($CompanyPurchaseOrder->supplier_id === $supplier->id) selected @endif value="{{ $supplier->id }}">{{ $supplier->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                    ...
                    <div class="col-md-12">
                        <table id="invoiceItemTable" class="table table-bordered">
                            ...
                            @foreach($CompanyPurchaseOrder->items as $item)
                                <tr class="tr_{{ $count }}">
                                    ...
                                    <td>
                                        ...
                                        <input type="text" name="row[{{ $count }}][model_name]"  value="<?php $Model = \App\Models\Ims\ItemCode::find($item->item_code_id); if($Model!=null){echo $Model->name;}?>" style="width: 100%">
                                    </td>

                                    <td>
                                        <input onkeyup="calTol({{ $count+1 }})" id="qty{{ $count }}" type="number" name="row[{{ $count }}][qty]" style="width: 100%; text-align: right" value="{{ $item->qty }}">
                                    </td>
                                    <td>
                                        <input id="price{{ $count }}" type="text" onkeyup="calTol({{ $count+1 }})" name="row[{{ $count }}][unit_price]" value="{{ number_format($item->unit_price,2) }}" style="width: 100%;text-align: right">
                                    </td>
                                    <td>
                                        <input id="tol{{ $count }}" type="text" readonly name="row[{{ $count }}][total]" style="width: 100%;text-align: right" value="{{ number_format(($item->qty * $item->unit_price),2) }}"> </td>
                                </tr>
                                <?php $count ++ ;?>
                            @endforeach
                            </tbody>
                            <tfoot>
                                <tr>
                                    <td colspan="4"></td>
                                    <td><input readonly style="width: 100%;text-align: right" value="{{ number_format($CompanyPurchaseOrder->total,2) }}"></td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                ...
                <div style="height: 100px" class="box-footer">
                    <a href="{{ url('/ims/company-purchase-order/'.$CompanyPurchaseOrder->id.'/print') }}" target="_blank" class="btn btn-app pull-right"><i style="color: #00a157" class="fa fa-print"></i> Print</a>
